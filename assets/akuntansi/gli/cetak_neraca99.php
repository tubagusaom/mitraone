...
// header
$a = session_id();
$SQL = "SELECT * FROM dbfn WHERE  norek NOT LIKE  '3110%' AND norek NOT LIKE '3210%' AND id = '".$a."'";
...
//looping aktiva LANCAR
	...
	$y = $y + 5;
	$jml_alancar=0;
$SQL = "SELECT * FROM dbfn WHERE  norek NOT LIKE  'MO1-%' AND norek NOT LIKE 'MO2-%'  AND id = '".$a."' and tipe = 'A' AND substr(norek,-4) = '0000' AND substr(norek,1,2) = 'AL' ORDER BY norek";
...
$SQL = "SELECT * FROM dbfn WHERE  norek NOT LIKE  'MO1-%' AND norek NOT LIKE 'MO2-%'  AND id = '".$a."' and tipe = 'A' AND substr(norek,-4) = '0000' AND substr(norek,1,2) = 'AT' ORDER BY norek";
$hasil = mysql_query($SQL, $dbh_jogjaide) or die(mysql_error());
while($baris = mysql_fetch_array($hasil)){
...
$SQL = "SELECT * FROM dbfn WHERE  norek NOT LIKE  'MO1%' AND norek NOT LIKE 'MO2%'  and norek LIKE 'KL%' AND id = '".$a."' and tipe = 'P' AND substr(norek,-4) = '0000' ORDER BY norek";
$hasil = mysql_query($SQL, $dbh_jogjaide);
while($baris = mysql_fetch_array($hasil)){
	$pdf->setY($y);
	//$pdf->cell(8,5,++$no, 1, 0, 'C');
	$pdf->cell(15,5,'', 0, 0, 'C');
		$pdf->cell(60,5,$baris['namarek'], 0, 0, 'L');
		$SQLc = "SELECT SUM(saldoawal) as saldoawal, SUM(debet) as debet, SUM(kredit) as kredit FROM dbfn WHERE norek NOT LIKE  'MO1-%' AND norek NOT LIKE 'MO2-%'  AND norek LIKE '".substr($baris['norek'], 0 ,4)."%' AND norek LIKE 'KL%' and id = '".$a."' ";
		$hasilc = mysql_query($SQLc, $dbh_jogjaide);
		$barisc = mysql_fetch_array($hasilc);
		$pdf->cell(28,5,number_format($barisc['saldoawal'],2,'.',','), 0, 0, 'R');
...
$SQLm = "SELECT SUM(saldoawal) as saldoawalm1, SUM(debet) as debetm1, SUM(kredit) as kreditm1 FROM dbfn WHERE norek LIKE  'MO1-%' AND norek  LIKE 'MO2-%'  and id = '".$a."' ";
