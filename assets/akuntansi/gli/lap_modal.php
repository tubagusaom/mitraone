...
// header
$a = session_id();
$SQL = "SELECT * FROM dbfn WHERE id = '".$a."' LIMIT 1";
...
$pdf->setY(60);
$SQL = " SELECT SUM(saldoawal+kredit-debet) FROM dbfn WHERE norek LIKE 'MO1%' AND id = '".$a."'";
$hasil = mysql_query($SQL);
$baris = mysql_fetch_array($hasil);
$modalawal = $baris[0];
...
$pdf->setY(70);
$SQL = " SELECT SUM(saldoawal+kredit-debet) FROM dbfn WHERE norek LIKE 'MO2%'  AND id = '".$a."'";
$hasil = mysql_query($SQL);
$baris = mysql_fetch_array($hasil);
$prive = $baris[0];
